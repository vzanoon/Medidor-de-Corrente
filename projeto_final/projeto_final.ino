...
int tensao = 220;
float potencia = 0;
float voltsporUnidade = 0.0039100684;// 4%1023 //parâmetros para calibração do sensor
...
void loop() {
  ...
  for(int i=samples; i>0; i--){
    sensorValue_aux = (analogRead(PINA0) - ref); // le o sensor no pino analogico A0 e ajusta o valor lido ja que a saída do sensor é (1023)vcc/2 para corrente = 0
    sensorValue += pow(sensorValue_aux,2); // somam os quadrados das leituras.
  }
  
  sensorValue = (sqrt(sensorValue/samples)) * voltsporUnidade; // finaliza o calculo da média quadratica e ajusta o valor lido para volts
